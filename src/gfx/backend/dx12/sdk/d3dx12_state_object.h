...
public:
		...
		for (UINT i = 0; i < m_Desc.NumSubobjects; i++)
		{
			...
			else if (m_SubobjectArray[i].Type == D3D12_STATE_SUBOBJECT_TYPE_GENERIC_PROGRAM)
			{
				...
				if (originalGenericProgramDesc->NumSubobjects > 0)
				{
					...
					{
						auto pWrapper						 = static_cast<const SUBOBJECT_WRAPPER*>(originalGenericProgramDesc->ppSubobjects[s]);
	...
	template <typename T, typename U> T* CreateSubobject(U&& arg)
	{
...
{
public:
	CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT() noexcept : m_Desc({})
	{
		Init();
	}
	CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT(const D3D12_RAYTRACING_SHADER_CONFIG& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT(const D3D12_RAYTRACING_SHADER_CONFIG& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_RAYTRACING_SHADER_CONFIG&() noexcept
	{
		return m_Desc;
	}
...
{
public:
	CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT() noexcept : m_Desc({})
	{
		Init();
	}
	CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT(const D3D12_RAYTRACING_PIPELINE_CONFIG& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT(const D3D12_RAYTRACING_PIPELINE_CONFIG& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_RAYTRACING_PIPELINE_CONFIG&() noexcept
	{
		return m_Desc;
	}
...
{
public:
	CD3DX12_RAYTRACING_PIPELINE_CONFIG1_SUBOBJECT() noexcept : m_Desc({})
	{
		Init();
	}
	CD3DX12_RAYTRACING_PIPELINE_CONFIG1_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_RAYTRACING_PIPELINE_CONFIG1_SUBOBJECT(const D3D12_RAYTRACING_PIPELINE_CONFIG1& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_RAYTRACING_PIPELINE_CONFIG1_SUBOBJECT(const D3D12_RAYTRACING_PIPELINE_CONFIG1& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_RAYTRACING_PIPELINE_CONFIG1&() noexcept
	{
		return m_Desc;
	}
...
#if defined(D3D12_SDK_VERSION) && (D3D12_SDK_VERSION >= 618)
//------------------------------------------------------------------------------------------------
class CD3DX12_GLOBAL_SERIALIZED_ROOT_SIGNATURE_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_GLOBAL_SERIALIZED_ROOT_SIGNATURE_SUBOBJECT() noexcept : m_Desc({})
	{
		Init();
	}
	CD3DX12_GLOBAL_SERIALIZED_ROOT_SIGNATURE_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator const D3D12_GLOBAL_SERIALIZED_ROOT_SIGNATURE&() const noexcept
	{
		return m_Desc;
	}
	operator D3D12_GLOBAL_SERIALIZED_ROOT_SIGNATURE&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_LOCAL_SERIALIZED_ROOT_SIGNATURE_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_LOCAL_SERIALIZED_ROOT_SIGNATURE_SUBOBJECT() noexcept : m_Desc({})
	{
		Init();
	}
	CD3DX12_LOCAL_SERIALIZED_ROOT_SIGNATURE_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator const D3D12_LOCAL_SERIALIZED_ROOT_SIGNATURE&() const noexcept
	{
		return m_Desc;
	}
	operator D3D12_LOCAL_SERIALIZED_ROOT_SIGNATURE&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_EXISTING_COLLECTION_BY_KEY_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	...
	operator const D3D12_EXISTING_COLLECTION_BY_KEY_DESC&() const noexcept
	{
		return m_Desc;
	}
...
{
public:
	CD3DX12_STATE_OBJECT_CONFIG_SUBOBJECT() noexcept : m_Desc({})
	{
		Init();
	}
	CD3DX12_STATE_OBJECT_CONFIG_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_STATE_OBJECT_CONFIG_SUBOBJECT(const D3D12_STATE_OBJECT_CONFIG& desc) noexcept : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_STATE_OBJECT_CONFIG_SUBOBJECT(const D3D12_STATE_OBJECT_CONFIG& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_STATE_OBJECT_CONFIG&() noexcept
	{
		return m_Desc;
	}
...
{
public:
	CD3DX12_NODE_MASK_SUBOBJECT() noexcept : m_Desc({})
	{
		Init();
	}
	CD3DX12_NODE_MASK_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_NODE_MASK_SUBOBJECT(const D3D12_NODE_MASK& desc) noexcept : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_NODE_MASK_SUBOBJECT(const D3D12_NODE_MASK& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_NODE_MASK&() noexcept
	{
		return m_Desc;
	}
...
#if defined(D3D12_SDK_VERSION) && (D3D12_SDK_VERSION >= 612)
//------------------------------------------------------------------------------------------------
class CD3DX12_STREAM_OUTPUT_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	...
	{
		m_soDecalEntries.emplace_back(D3D12_SO_DECLARATION_ENTRY{entry.Stream, m_Strings.LocalCopy(entry.SemanticName), entry.SemanticIndex, entry.StartComponent, entry.ComponentCount, entry.OutputSlot});
		m_Desc.NumEntries++;
		// Below: using ugly way to get pointer in case .data() is not defined
	...
	void SetSODeclEntries(const D3D12_SO_DECLARATION_ENTRY* soDeclEntries, UINT numEntries)
	{
		...
		{
			m_soDecalEntries[i] =
				D3D12_SO_DECLARATION_ENTRY{soDeclEntries[i].Stream, m_Strings.LocalCopy(soDeclEntries[i].SemanticName), soDeclEntries[i].SemanticIndex, soDeclEntries[i].StartComponent, soDeclEntries[i].ComponentCount, soDeclEntries[i].OutputSlot};
		}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_BLEND_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_BLEND_SUBOBJECT() : m_Desc(CD3DX12_BLEND_DESC(D3D12_DEFAULT))
	{
		Init();
	}
	CD3DX12_BLEND_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(CD3DX12_BLEND_DESC(D3D12_DEFAULT))
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_BLEND_SUBOBJECT(const D3D12_BLEND_DESC& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_BLEND_SUBOBJECT(const D3D12_BLEND_DESC& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_BLEND_DESC&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_RASTERIZER_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_RASTERIZER_SUBOBJECT() : m_Desc(CD3DX12_RASTERIZER_DESC2(D3D12_DEFAULT))
	{
		Init();
	}
	CD3DX12_RASTERIZER_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(CD3DX12_RASTERIZER_DESC2(D3D12_DEFAULT))
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_RASTERIZER_SUBOBJECT(const D3D12_RASTERIZER_DESC2& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_RASTERIZER_SUBOBJECT(const D3D12_RASTERIZER_DESC2& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_RASTERIZER_DESC2&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_DEPTH_STENCIL2_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_DEPTH_STENCIL2_SUBOBJECT() : m_Desc(CD3DX12_DEPTH_STENCIL_DESC2(D3D12_DEFAULT))
	{
		Init();
	}
	CD3DX12_DEPTH_STENCIL2_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(CD3DX12_DEPTH_STENCIL_DESC2(D3D12_DEFAULT))
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_DEPTH_STENCIL2_SUBOBJECT(const D3D12_DEPTH_STENCIL_DESC2& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_DEPTH_STENCIL2_SUBOBJECT(const D3D12_DEPTH_STENCIL_DESC2& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	{
		m_Desc.FrontFace = {frontFace.StencilFailOp, frontFace.StencilDepthFailOp, frontFace.StencilPassOp, frontFace.StencilFunc, frontFace.StencilReadMask, frontFace.StencilWriteMask};
	}
	void SetBackFace(D3D12_DEPTH_STENCILOP_DESC1 backFace)
	{
		m_Desc.BackFace = {backFace.StencilFailOp, backFace.StencilDepthFailOp, backFace.StencilPassOp, backFace.StencilFunc, backFace.StencilReadMask, backFace.StencilWriteMask};
	}
	...
	operator D3D12_DEPTH_STENCIL_DESC2&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_INPUT_LAYOUT_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
...
//------------------------------------------------------------------------------------------------
class CD3DX12_IB_STRIP_CUT_VALUE_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_IB_STRIP_CUT_VALUE_SUBOBJECT() : m_Desc(D3D12_INDEX_BUFFER_STRIP_CUT_VALUE_DISABLED)
	{
		Init();
	}
	CD3DX12_IB_STRIP_CUT_VALUE_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(D3D12_INDEX_BUFFER_STRIP_CUT_VALUE_DISABLED)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_IB_STRIP_CUT_VALUE_SUBOBJECT(D3D12_INDEX_BUFFER_STRIP_CUT_VALUE desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_IB_STRIP_CUT_VALUE_SUBOBJECT(D3D12_INDEX_BUFFER_STRIP_CUT_VALUE desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_INDEX_BUFFER_STRIP_CUT_VALUE&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_PRIMITIVE_TOPOLOGY_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_PRIMITIVE_TOPOLOGY_SUBOBJECT() : m_Desc(D3D12_PRIMITIVE_TOPOLOGY_TYPE_UNDEFINED)
	{
		Init();
	}
	CD3DX12_PRIMITIVE_TOPOLOGY_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(D3D12_PRIMITIVE_TOPOLOGY_TYPE_UNDEFINED)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_PRIMITIVE_TOPOLOGY_SUBOBJECT(D3D12_PRIMITIVE_TOPOLOGY_TYPE desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_PRIMITIVE_TOPOLOGY_SUBOBJECT(D3D12_PRIMITIVE_TOPOLOGY_TYPE desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_PRIMITIVE_TOPOLOGY_TYPE&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_RENDER_TARGET_FORMATS_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_RENDER_TARGET_FORMATS_SUBOBJECT() : m_Desc({})
	{
		Init();
	}
	CD3DX12_RENDER_TARGET_FORMATS_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_RENDER_TARGET_FORMATS_SUBOBJECT(const D3D12_RT_FORMAT_ARRAY& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_RENDER_TARGET_FORMATS_SUBOBJECT(const D3D12_RT_FORMAT_ARRAY& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_RT_FORMAT_ARRAY&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_DEPTH_STENCIL_FORMAT_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_DEPTH_STENCIL_FORMAT_SUBOBJECT() : m_Desc(DXGI_FORMAT_UNKNOWN)
	{
		Init();
	}
	CD3DX12_DEPTH_STENCIL_FORMAT_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(DXGI_FORMAT_UNKNOWN)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_DEPTH_STENCIL_FORMAT_SUBOBJECT(DXGI_FORMAT desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_DEPTH_STENCIL_FORMAT_SUBOBJECT(DXGI_FORMAT desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_SAMPLE_DESC_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_SAMPLE_DESC_SUBOBJECT() : m_Desc({1, 0})
	{
		Init();
	}
	CD3DX12_SAMPLE_DESC_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc({1, 0})
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_SAMPLE_DESC_SUBOBJECT(const DXGI_SAMPLE_DESC& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_SAMPLE_DESC_SUBOBJECT(const DXGI_SAMPLE_DESC& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_FLAGS_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_FLAGS_SUBOBJECT() : m_Desc(D3D12_PIPELINE_STATE_FLAG_NONE)
	{
		Init();
	}
	CD3DX12_FLAGS_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(D3D12_PIPELINE_STATE_FLAG_NONE)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_FLAGS_SUBOBJECT(D3D12_PIPELINE_STATE_FLAGS desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_FLAGS_SUBOBJECT(D3D12_PIPELINE_STATE_FLAGS desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_PIPELINE_STATE_FLAGS&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_VIEW_INSTANCING_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	...
	void AddViewInstanceLocation(D3D12_VIEW_INSTANCE_LOCATION viewInstanceLocation)
	{
		...
		m_viewInstanceLocations.emplace_back(D3D12_VIEW_INSTANCE_LOCATION{viewInstanceLocation.ViewportArrayIndex, viewInstanceLocation.RenderTargetArrayIndex});
		// Below: using ugly way to get pointer in case .data() is not defined
...
//------------------------------------------------------------------------------------------------
class CD3DX12_DEPTH_STENCIL_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_DEPTH_STENCIL_SUBOBJECT() : m_Desc(CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT))
	{
		Init();
	}
	CD3DX12_DEPTH_STENCIL_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT))
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_DEPTH_STENCIL_SUBOBJECT(const D3D12_DEPTH_STENCIL_DESC& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_DEPTH_STENCIL_SUBOBJECT(const D3D12_DEPTH_STENCIL_DESC& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_DEPTH_STENCIL_DESC&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_DEPTH_STENCIL1_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_DEPTH_STENCIL1_SUBOBJECT() : m_Desc(CD3DX12_DEPTH_STENCIL_DESC1(D3D12_DEFAULT))
	{
		Init();
	}
	CD3DX12_DEPTH_STENCIL1_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(CD3DX12_DEPTH_STENCIL_DESC1(D3D12_DEFAULT))
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_DEPTH_STENCIL1_SUBOBJECT(const D3D12_DEPTH_STENCIL_DESC1& desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_DEPTH_STENCIL1_SUBOBJECT(const D3D12_DEPTH_STENCIL_DESC1& desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	...
	operator D3D12_DEPTH_STENCIL_DESC1&() noexcept
	{
		return m_Desc;
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_SAMPLE_MASK_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
	CD3DX12_SAMPLE_MASK_SUBOBJECT() : m_Desc(0xffffffffu)
	{
		Init();
	}
	CD3DX12_SAMPLE_MASK_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(0xffffffffu)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
	CD3DX12_SAMPLE_MASK_SUBOBJECT(UINT desc) : m_Desc(desc)
	{
		Init();
	}
	CD3DX12_SAMPLE_MASK_SUBOBJECT(UINT desc, CD3DX12_STATE_OBJECT_DESC& ContainingStateObject) : m_Desc(desc)
	{
		Init();
		AddToStateObject(ContainingStateObject);
	}
...
//------------------------------------------------------------------------------------------------
class CD3DX12_GENERIC_PROGRAM_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
...
public:
	CD3DX12_NODE_HELPER_BASE(const Backreference& BackRef) : m_BackRef(BackRef)
	{
	}
...
{
public:
	CD3DX12_SHADER_NODE(const Backreference& BackRef, LPCWSTR _Shader = nullptr) : CD3DX12_NODE_HELPER_BASE(BackRef)
	{
	...
	LPCWSTR GetShaderName() const
	{
		return GetNode()->Shader.Shader;
	}
};
...
class CD3DX12_BROADCASTING_LAUNCH_NODE_OVERRIDES : public CD3DX12_NODE_HELPER_BASE
{
public:
	CD3DX12_BROADCASTING_LAUNCH_NODE_OVERRIDES(const Backreference& BackRef, LPCWSTR _Shader = nullptr) : CD3DX12_NODE_HELPER_BASE(BackRef), m_NodeOutputOverrides(&Overrides.pOutputOverrides, &Overrides.NumOutputOverrides)
	{
		Overrides								   = {};
...
// need to be overridden.  If overrides are not needed, just use CD3DX12_COMPUTE_SHADER_NODE
class CD3DX12_COALESCING_LAUNCH_NODE_OVERRIDES : public CD3DX12_NODE_HELPER_BASE
{
public:
	CD3DX12_COALESCING_LAUNCH_NODE_OVERRIDES(const Backreference& BackRef, LPCWSTR _Shader = nullptr) : CD3DX12_NODE_HELPER_BASE(BackRef), m_NodeOutputOverrides(&Overrides.pOutputOverrides, &Overrides.NumOutputOverrides)
	{
		Overrides								 = {};
		D3D12_NODE* pNode						 = GetNode();
		pNode->NodeType							 = D3D12_NODE_TYPE_SHADER;
...
// need to be overridden.  If overrides are not needed, just use CD3DX12_COMPUTE_SHADER_NODE
class CD3DX12_THREAD_LAUNCH_NODE_OVERRIDES : public CD3DX12_NODE_HELPER_BASE
{
public:
	CD3DX12_THREAD_LAUNCH_NODE_OVERRIDES(const Backreference& BackRef, LPCWSTR _Shader = nullptr) : CD3DX12_NODE_HELPER_BASE(BackRef), m_NodeOutputOverrides(&Overrides.pOutputOverrides, &Overrides.NumOutputOverrides)
	{
		Overrides							 = {};
		D3D12_NODE* pNode					 = GetNode();
		pNode->NodeType						 = D3D12_NODE_TYPE_SHADER;
...
class CD3DX12_COMMON_COMPUTE_NODE_OVERRIDES : public CD3DX12_NODE_HELPER_BASE
{
public:
	CD3DX12_COMMON_COMPUTE_NODE_OVERRIDES(const Backreference& BackRef, LPCWSTR _Shader = nullptr) : CD3DX12_NODE_HELPER_BASE(BackRef), m_NodeOutputOverrides(&Overrides.pOutputOverrides, &Overrides.NumOutputOverrides)
	{
		Overrides							 = {};
		D3D12_NODE* pNode					 = GetNode();
		pNode->NodeType						 = D3D12_NODE_TYPE_SHADER;
...
//------------------------------------------------------------------------------------------------
class CD3DX12_WORK_GRAPH_SUBOBJECT : public CD3DX12_STATE_OBJECT_DESC::SUBOBJECT_HELPER_BASE
{
public:
